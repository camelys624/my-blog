...
function paintHuiwen(ctx, props) {
  ...
  for (let i = 0; i < Math.round((width) / (lineWidth * 12)); i++) {
    offset = i * huiwenWidth + 1 - lineWidth * (i - 1);
    if (i === 0) {
    ...
    ctx.lineTo(offset + huiwenWidth - lineWidth * 3, beginPoint + lineWidth * 3);
    ctx.lineTo(offset + huiwenWidth - lineWidth * 3, beginPoint + lineWidth * 9);
    ctx.lineTo(offset + lineWidth * 4, beginPoint + lineWidth * 9);
    ctx.lineTo(offset + lineWidth * 4, beginPoint + lineWidth * 7);
    ctx.lineTo(offset + huiwenWidth - lineWidth * 5, beginPoint + lineWidth * 7);
    ctx.lineTo(offset + huiwenWidth - lineWidth * 5, beginPoint + lineWidth * 5);
    ctx.lineTo(offset + lineWidth * 2, beginPoint + lineWidth * 5);
    ctx.lineTo(offset + lineWidth * 2, beginPoint + lineWidth * 11);
    ctx.lineTo(offset + huiwenWidth - lineWidth, beginPoint + lineWidth * 11);
    if (i === Math.round((width - 8) / (lineWidth * 12)) - 1) {
      ctx.lineTo(offset + huiwenWidth - lineWidth, beginPoint + lineWidth * 3);
    }
  }
}
